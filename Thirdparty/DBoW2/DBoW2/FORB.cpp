...
namespace DBoW2
{
    ...
    // note 从二进制文件中加载描述符
    void FORB::loadORBDescriptors(const std::string &filename, std::vector<TDescriptor> &descriptors)
    {
        ifstream f;
        ...
        if (!f.is_open())
        {
            throw std::runtime_error("无法打开文件以读取ORB描述符");
        }
    ...
    // --------------------------------------------------------------------------
    // note 将描述符写入二进制文件
    void FORB::saveORBDescriptors(const std::string &filename, const std::vector<TDescriptor> &descriptors)
    {
        ofstream f;
        f.open(filename, ios::binary);
        ...
        if (!f.is_open())
        {
            throw std::runtime_error("无法打开文件以写入ORB描述符");
        }
