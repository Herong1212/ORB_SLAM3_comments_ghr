...
#define FRAME_GRID_COLS 64
    ...
    public:
        ...
        // note：每个frame类里都有三个关于词袋的成员变量
        // Vocabulary used for relocalization.
        ORBVocabulary *mpORBvocabulary; // notice1：使用的字典的指针，字典用于全局重定位和闭环检测
        ...
        // Bag of Words Vector structures. --- 注意：每个关键帧各有一个 mBowVec 和一个 mFeatVec。
        DBoW2::BowVector mBowVec;      // notice2：bag of words vector，就是描述整幅图像的一系列视觉词汇，视觉单词的id和它的权重值，如图像 A = {(3,0.5),(7,0.2),(15,0.8),(21,0.4)} → vA
        DBoW2::FeatureVector mFeatVec; // notice3：feature vector，记录了图像中每一个特征点与词汇表中的特征“单词”之间的具体对应关系，也就是哪些特征点属于哪个节点，如图像 A={(5,[2,5,9]),(8,[0,3]),(12,[1,4,6,7])}，

        // ORB descriptor, each row associated to a keypoint.
        cv::Mat mDescriptors; // notice：原始特征描述子矩阵，其中每一行是一个特征点的描述子。
        ...
        IMU::Preintegrated *mpImuPreintegrated; // 当前普通帧到前一【普通帧】的预积分。
