...
namespace ORB_SLAM3
{
    ...
    namespace IMU
    {
        ...
        public:
            // 构造函数，初始化加速度、角速度和时间戳
            // 第一个构造函数接受六个浮点数参数（加速度和角速度的三个分量）以及一个时间戳，使用这些参数初始化类成员 a（加速度向量）和 w（角速度向量）。
            ...
            // 使用 cv::Point3f 构造函数
            // 第二个构造函数接受两个 cv::Point3f 类型的参数（分别表示加速度和角速度）和一个时间戳。这种方式可以方便地将 OpenCV 的三维点转换为 Point 类。
        ...
        public:
            Eigen::Vector3f a;              // 加速度向量
            Eigen::Vector3f w;              // 角速度向量
            double t;                       // 时间戳，记录测量数据的时间点。
            EIGEN_MAKE_ALIGNED_OPERATOR_NEW // 对齐宏，确保类的动态分配（new）时对齐，符合 Eigen 的要求，以避免潜在的性能问题和错误。
        };
