...
namespace ORB_SLAM3
{
        ...
        const int minLevel, const int maxLevel, const bool bRight) const
    {
        ...
        // Step 1 计算半径为 r 圆左右上下边界所在的网格列和行的 id
        // 查找半径为 r 的圆左侧边界所在网格列坐标。这个地方有点绕，慢慢理解下：
        ...
        // (x-mnMinX-r)，可以看做是从图像的左边界 mnMinX 到半径 r 的圆的左边界区域占的像素列数
        // 两者相乘，就是求出那个半径为 r 的圆的左侧边界在哪个网格列中
        // 保证 nMinCellX 结果大于等于 0
        const int nMinCellX = max(0, (int)floor((x - mnMinX - factorX) * mfGridElementWidthInv));

        // 如果最终求得的圆的左边界所在的网格列超过了设定了上限，那么就说明计算出错，找不到符合要求的特征点，返回空 vector
        ...
        // Step 2 遍历圆形区域内的所有网格，寻找满足条件的候选特征点，并将其 index 放到输出里
        for (int ix = nMinCellX; ix <= nMaxCellX; ix++)
        {
            for (int iy = nMinCellY; iy <= nMaxCellY; iy++)
            {
                ...
                for (size_t j = 0, jend = vCell.size(); j < jend; j++)
                {
                    ...
                    // 如果 x 方向和 y 方向的距离都在指定的半径之内，存储其 index 为候选特征点
    ...
    /**
     * @brief 计算当前帧特征点对应的词袋 Bow，主要是 mBowVec 和 mFeatVec
     *
     */
    void Frame::ComputeBoW()
    {
        ...
        if (mBowVec.empty())
        {
            // 将描述子 mDescriptors 转换为 DBOW 要求的输入格式，即把 mDescriptors push_back 到 currentDesc 中
            // DBoW库通常要求输入的是一个vector<cv::Mat>格式的描述子向量，而不是 cv::Mat 的矩阵，因此需要先做格式转换，即将矩阵的每个元素单独取出来作为一个向量元素。
            vector<cv::Mat> vCurrentDesc = Converter::toDescriptorVector(mDescriptors); // notice：转换后的描述子向量，每个 cv::Mat 表示一个特征点的描述子，这种格式符合 DBoW 库的要求。

            // 将转换出来的每个当前特征点的描述子向量转换成词袋向量 mBowVec 以及节点与索引的特征向量 mFeatVec
            mpORBvocabulary->transform(vCurrentDesc, // 当前特征点的描述子 vector
                                       mBowVec,      // 输出，词袋向量，记录的是单词的 id 及其对应权重 TF-IDF 值
                                       mFeatVec,     // 输出，记录 node id 及其对应的图像 feature 对应的索引
                                       4);           // 4 代表字典树中的从叶子节点向上数的第4层，见下段代码中的注释。
        }
    }
