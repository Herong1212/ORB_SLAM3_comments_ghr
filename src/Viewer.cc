...
namespace ORB_SLAM3
{
    ...
    // note：主函数！负责主渲染线程的函数，该函数用于利用Pangolin绘制SLAM系统中的关键元素，如：
    void Viewer::Run()
    {
        mbFinished = false;
        mbStopped = false;

        // 启动 Pangolin 窗口和 3D 视图
        ...
        // ps：定义了一个3D渲染窗口，以显示相机的轨迹、地图点、关键帧等信息。
        pangolin::Var<bool> menuShowOptLba("menu.Show LBA opt", false, true); // ps：定义了一个Pangolin菜单变量，用于控制是否显示“LBA优化”。

        // Define Camera Render Object (for view / scene browsing) --- 相机视图初始化，详见之前代码
        // ps：定义一个摄像机的渲染状态，用于指定相机的投影矩阵和视角。这些参数决定了视图的观察角度和投影方式。
        pangolin::OpenGlRenderState s_cam(
            pangolin::ProjectionMatrix(1024, 768, mViewpointF, mViewpointF, 512, 389, 0.1, 1000),
            pangolin::ModelViewLookAt(mViewpointX, mViewpointY, mViewpointZ, 0, 0, 0, 0.0, -1.0, 0.0));

        // ps：创建一个显示窗口（View），并将其与3D处理器（Handler3D）关联。这段代码配置了窗口的大小、位置及处理方式。
        ...
        while (1)
        // 也可以用:while (!pangolin::ShouldQuit())
        {
            ...
            mpMapDrawer->DrawCurrentCamera(Twc); // 3. 绘制当前相机位姿
            if (menuShowKeyFrames || menuShowGraph || menuShowInertialGraph || menuShowOptLba)
                mpMapDrawer->DrawKeyFrames(menuShowKeyFrames, menuShowGraph, menuShowInertialGraph, menuShowOptLba); // 2. 绘制关键帧
            if (menuShowPoints)
                mpMapDrawer->DrawMapPoints(); // 1. 绘制地图点

            pangolin::FinishFrame();

            cv::Mat toShow;
            cv::Mat im = mpFrameDrawer->DrawFrame(trackedImageScale); // 4. 绘制普通帧
