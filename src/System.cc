...
namespace ORB_SLAM3
{
     ...
     * @param initFr initFr 表示初始化帧的 id, 开始设置为 0
     * @param strSequence 序列名，在【跟踪线程】和【局部建图线程】用得到
     */
                                                                                         ...
                                                                                         mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
    {
        ...
        {
            settings_ = nullptr;
            // 尝试从配置文件中读取 System.LoadAtlasFromFile 节点（即地图加载路径）。
            // ? 但是配置文件里都没有这两个字段啊，而且基本都是 1.0 版本的
        ...
        // ORBSLAM3 新加的多地图管理功能，这里加载 Atlas 标识符
        ...
        // 创建跟踪线程（主线程），不会立刻开启，会在对图像和 imu 预处理后在 main 主线程中执行
        cout << "Seq. Name: " << strSequence << endl;
        mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                                 mpAtlas, mpKeyFrameDatabase, strSettingsFile, mSensor, settings_, strSequence);

        // 创建并开启 local mapping 线程
        ...
        // 设置线程间的指针（Set pointers between threads）
        ...
        // 创建并开启显示线程（Initialize the Viewer thread and launch）
        if (bUseViewer)
        // if(false) // TODO
        {
