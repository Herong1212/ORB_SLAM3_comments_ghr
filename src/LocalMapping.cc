...
namespace ORB_SLAM3
{
    ...
    void LocalMapping::Run()
    {
        ...
        while (1)
        {
            ...
            // 等待处理的关键帧列表不为空 并且 imu 正常
            // 等待处理的关键帧列表不为空 --- 即队列里有已经存在的关键帧，就需要先处理完已经存在的，再去接收新的
            // Check if there are keyframes in the queue
            if (CheckNewKeyFrames() && !mbBadImu) // todo 检查有没有关键帧，如果有的话开始依次执行下面的代码
            {
#ifdef REGISTER_TIMES
                double timeLBA_ms = 0;
                ...
                if (!CheckNewKeyFrames())
                {
                    ...
                    SearchInNeighbors(); // 搜索领域匹配，检查并融合当前关键帧与相邻帧（两级相邻）重复的地图点
                }
                ...
                mpLoopCloser->InsertKeyFrame(mpCurrentKeyFrame); // 将关键帧插入到闭环线程
            ...
            usleep(3000);
            // std::this_thread::sleep_for(std::chrono::milliseconds(3));  ORB-SLAM2中的用法
        }
    ...
    /**
     * @brief 处理列表中的关键帧，包括计算 BoW、更新观测、描述子、共视图，插入到地图等
     */
    void LocalMapping::ProcessNewKeyFrame()
    {
        // Step 1：从缓冲队列中取出一帧关键帧
        // 该关键帧队列是 Tracking 线程向 LocalMapping 中插入的关键帧组成
        {
            unique_lock<mutex> lock(mMutexNewKFs);
        ...
        // Step 2：计算该关键帧特征点的 Bow 信息
        // Compute Bags of Words structures
        mpCurrentKeyFrame->ComputeBoW();

        // Step 3：当前处理关键帧中有效的地图点，更新 normal，描述子等信息
    ...
    void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
    {
        ...
        // 同样内容再构建一个和 lpKF 一样的容器 vpKF
        vector<KeyFrame *> vpKF(lpKF.begin(), lpKF.end());
        if (vpKF.size() < nMinKF)
            return;

        // case3：检查是否满足头尾关键帧时间戳之差的条件，不满足直接返回；
        ...
        {
            ProcessNewKeyFrame();
            vpKF.push_back(mpCurrentKeyFrame); // 将 mpCurrentKeyFrame 分别放入 vector 和 list 容器中
            lpKF.push_back(mpCurrentKeyFrame);
        }

        // Step3：正式 IMU 初始化
        const int N = vpKF.size();
        IMU::Bias b(0, 0, 0, 0, 0, 0); // 零偏初始值设为 0

        // Compute and KF velocities mRwg estimation
        // 在 IMU 连一次初始化都没有做的情况下
        if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
        {
            Eigen::Matrix3f Rwg; // 待求的重力方向
            Eigen::Vector3f dirG;
            dirG.setZero();
            ...
            for (vector<KeyFrame *>::iterator itKF = vpKF.begin(); itKF != vpKF.end(); itKF++)
            {
                ...
                have_imu_num++; // ? 这部分相关代码应该是调试用的吧！
                // * 核心代码：初始化时关于速度的预积分定义 Ri.t() * (s * Vj - s * Vi - Rwg * g * tij)
            ...
            if (have_imu_num < 6)
            {
                cout << "imu初始化失败, 由于带有 imu 预积分信息的关键帧数量太少" << endl;
                bInitializing = false;
                mbBadImu = true;
                return;
            }

            // * dirG = sV1 - sVn + n * Rwg * g * t，dirG 的作用：
            // 归一化，约等于重力在世界坐标系下的方向
            dirG = dirG / dirG.norm();
            // 原本的重力方向
            ...
            // v / nv 表示垂直于两个向量的轴，ang 表示转的角度，组成角轴，即：先计算旋转向量，再除去角轴大小
            Eigen::Vector3f vzg = v * ang / nv;
            // 获得从重力方向（重力坐标系）到世界坐标系的旋转矩阵的初值
            Rwg = Sophus::SO3f::exp(vzg).matrix(); // Rwg --- 重力方向
        ...
        // 计算残差及偏置差，优化尺度重力方向及速度偏置，偏置先验为 0，双目时不优化尺度
        ...
        // 到此时为止，前面做的东西没有改变 map

        // Before this line we are not changing the map
        {
            unique_lock<mutex> lock(mpAtlas->GetCurrentMap()->mMutexMapUpdate);

            // 尺度变化超过设定值，或者非单目时（无论带不带 imu，但这个函数只在带 imu 时才执行，所以这个可以理解为双目 imu）
            if ((fabs(mScale - 1.f) > 0.00001) || !mbMonocular)
            {
                // 恢复重力方向与尺度信息
                Sophus::SE3f Twg(mRwg.cast<float>().transpose(), Eigen::Vector3f::Zero());
                mpAtlas->GetCurrentMap()->ApplyScaledRotation(Twg, mScale, true); // note1：重要函数！

                // 更新跟踪线程中普通帧的位姿，主要是当前帧与上一帧
                mpTracker->UpdateFrameIMU(mScale, vpKF[0]->GetImuBias(), mpCurrentKeyFrame); // vpKF[0]->GetImuBias() --- 偏置量 // note2：重要函数！
            }
        ...
        if (bFIBA)
        {
            // 承接上一步纯 imu 优化，按照之前的结果更新了尺度信息及适应重力方向，所以要结合地图进行一次视觉加 imu 的全局优化，这次带了 MP 等信息
            // ! 1.0 版本里面不直接赋值了，而是将所有优化后的信息保存到变量里面
            if (priorA != 0.f)
                Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(), 100, false, mpCurrentKeyFrame->mnId, NULL, true, priorG, priorA); // note：整体优化，所有都优化了！
            else
                Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(), 100, false, mpCurrentKeyFrame->mnId, NULL, false);
        }

        // 👇都是来更新地图中关键帧的位姿和地图点的坐标，删除并清空局部建图线程中缓存的关键帧
        ...
        // 处理一下新来的关键帧，这些关键帧没有参与优化，但是这部分 bInitializing 为 true，只在第2次跟第3次初始化会有新的关键帧进来
        // 这部分关键帧也需要被更新
        while (CheckNewKeyFrames())
        {
            ProcessNewKeyFrame();
        ...
        // 9. 再有新的来就不要了~不然陷入无限套娃了，即：里面存放的是来自 Local 线程，但还没处理的关键帧
        for (list<KeyFrame *>::iterator lit = mlNewKeyFrames.begin(), lend = mlNewKeyFrames.end(); lit != lend; lit++)
        {
            (*lit)->SetBadFlag();
            delete *lit;
        ...
        // * 全部结束后，这里把  bInitializing 再置为 false，防止再次进行初始化。
    ...
    void LocalMapping::ScaleRefinement()
    {
        ...
        // Step2. 更新旋转与尺度
        // 待优化变量的初值
        mRwg = Eigen::Matrix3d::Identity(); // mRwg --- 重力方向
        mScale = 1.0;                       // 尺度
        ...
        // Step3.1 如果尺度更新较多，或是在双目imu情况下更新地图
        // * 0.4 版本这个值为0.00001
