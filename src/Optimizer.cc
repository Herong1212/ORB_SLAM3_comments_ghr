...
namespace ORB_SLAM3
{
    ...
    {
        // 该优化函数主要用于 Tracking 线程中：运动跟踪、参考帧跟踪、地图跟踪、重定位

        // Step 1：构造 g2o 优化器, BlockSolver_6_3表示：位姿 _PoseDim 为 6 维，路标点 _LandmarkDim 是 3 维
        ...
        const int N = pFrame->N; // pFrame->N --- 当前帧的特征点数量
        ...
        // 自由度为 2 的卡方分布，显著性水平为 0.05，对应的临界阈值 5.991
        // 可以理解为卡方值高于 5.991 95% 的几率维外点
    ...
    int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
    {
        ...
        */
        ...
        // 构造一个 ConstraintPoseImu 对象，为下一帧提供先验约束
        // 构造对象所使用的参数是当前帧 P、V、BG、BA 的估计值和边缘化后的H矩阵
        pFrame->mpcpi = new ConstraintPoseImu(VP->estimate().Rwb, VP->estimate().twb, VV->estimate(), VG->estimate(), VA->estimate(), H.block<15, 15>(15, 15));
        // 下一帧使用的 EdgePriorPoseImu 参数来自于此
        delete pFp->mpcpi;
        pFp->mpcpi = NULL;
    ...
    // NOTE：imu 初始化优化
    /**
     * @brief imu 初始化优化，LocalMapping::InitializeIMU 中使用，其中kf的位姿是固定不变的
     * 优化目标 重力方向，尺度，速度与偏置
     * @param pMap 地图
    ...
    // notice：进行 visual inertial ba
    /**
     * @brief 这里面进行 visual inertial ba
        ...
        KeyFrame *pCurrKF, KeyFrame *pMergeKF, bool *pbStopFlag, Map *pMap, LoopClosing::KeyFrameAndPose &corrPoses)
    {
        ...
        // Step3. 帧弄完了该弄点了，将优化的帧的点存入lLocalMapPoints (所有被可优化关键帧看到的点)
