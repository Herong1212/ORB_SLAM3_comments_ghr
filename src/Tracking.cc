 ...
 * Copyright (C) 2014-2016 Raúl MuOpenCV_INCLUDE_DIRS
...
namespace ORB_SLAM3
{
    ...
    {
        mDistCoef = cv::Mat::zeros(4, 1, CV_32F); // 用于初始化相机的畸变系数矩阵
        cout << endl
             << "Camera Parameters: " << endl;
        bool b_miss_params = false; // 指示是否在参数中遗漏了某些值
        ...
        // case2：Kannala-Brandt 8 畸变模型（鱼眼相机）
        else if (sCameraName == "KannalaBrandt8")
        {
            ...
            // TODO1 Camera calibration parameters
            cv::FileNode node = fSettings["Camera.fx"];
            if (!node.empty() && node.isReal())
            {
                fx = node.real();
    ...
    {
        std::cout << "GrabImageStereo666" << endl;
    ...
    Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
    {
        ...
        // 返回当前帧的位姿
        return mCurrentFrame.GetPose();
        // return mCurrentFrame.mTcw.clone(); 视频里是这个，世界坐标系到当前坐标系的位姿变换
    }
    ...
    void Tracking::Track()
    {
        ...
        if (bStepByStep)
        {
            std::cout << "Tracking: Waiting to the next step" << std::endl; // 用于提示用户或开发者，程序正在等待进入下一步。

            while (!mbStep && bStepByStep)
                usleep(500); // 使程序暂停 500 微秒（0.5 毫秒），这样做是为了避免循环空转（busy-waiting），减少 CPU 的无效消耗。
            mbStep = false;
        }

        // Step 1 如局部建图里认为 IMU 有问题，重置当前活跃地图 --- 这里不用管，一般很少出现
        ...
        if (mState != NO_IMAGES_YET)
        {
            ...
            // case2：如果当前图像时间戳和前一帧图像时间戳【大于1s -- 有 30 帧了】，说明时间戳明显跳变了，重置地图后直接返回
            else if (mCurrentFrame.mTimeStamp > mLastFrame.mTimeStamp + 1.0)
            {
                // cout << mCurrentFrame.mTimeStamp << ", " << mLastFrame.mTimeStamp << endl; // 打 log 调试用
                ...
                {
                    // 如果当前地图完成 imu 初始化，则完成第 1 阶段初始化
                    if (mpAtlas->isImuInitialized())
                    {
                        cout << "Timestamp jump detected. State set to LOST. Reseting IMU integration..." << endl;

                        // IMU 完成第 3 次初始化（在 localmapping 线程里）
                    ...
                    {
                        // 如果当前子图中 imu 还没有初始化，重置 active 地图
                        cout << "Timestamp jump detected, before IMU initialization. Reseting..." << endl;
                        mpSystem->ResetActiveMap();
                    }
                }
                else // 非 IMU 模式，放弃追踪，直接返回
                {
                    return;
                }
            }
        ...
        // Step 3 IMU 模式下设置 IMU 的 Bias 参数，还要保证上一帧存在
        if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && mpLastKeyFrame)
            mCurrentFrame.SetNewBias(mpLastKeyFrame->GetImuBias()); // 使用上一帧的 bias 作为当前帧的初值，认为 bias 在两帧间不变
        ...
        // Step 5 初始化，从这里开始对照流程图来看
        // case1：没有初始化的情况，第一帧进来走这里
        if (mState == NOT_INITIALIZED)
        {
            ...
            // notice 这个代码片段的核心功能是确保在SLAM系统的初始化过程中，正确记录下第一帧的ID。
            if (mpAtlas->GetAllMaps().size() == 1)
            {
                // 如果当前地图是第一个地图，记录当前帧 id 为第一帧
        ...
        else
        {
            ...
            if (!mbOnlyTracking)
            {
                ...
                if (mState == OK)
                {
                    ...
                    // Step 6.2 运动模型是空的并且imu未初始化 或 刚完成重定位，跟踪参考关键帧；否则恒速模型跟踪
                    // 第一个条件：如果运动模型为空并且imu未初始化，说明是刚开始第一帧跟踪，或者已经跟丢了。
                    // 第二个条件：如果当前帧紧紧地跟着在重定位的帧的后面，我们用重定位帧来恢复位姿
                    // mnLastRelocFrameId 上一次重定位的那一帧。mCurrentFrame.mnId < mnLastRelocFrameId + 2 说明当前帧紧跟在上一次重定位帧的后面一帧，如5，6帧
                    ...
                    // notice：新增了一个状态 RECENTLY_LOST，主要是结合IMU看看能不能拽回来
                    // Step 6.3 如果经过跟踪参考关键帧、恒速模型跟踪都失败的话，并满足一定条件就要标记为 RECENTLY_LOST 或 LOST
                    if (!bOK)
                    {
                        // 条件1：如果当前帧距离上次重定位成功不到 1s
                        //        mnFramesToResetIMU 表示经过多少帧后可以重置 IMU，一般设置为和帧率相同，对应的时间是1s
                ...
                else // 跟踪不正常按照下面处理
                {
                    ...
                    else if (mState == LOST)
                    {
                        ...
                        {
                            // 情况1：当前活跃地图中的关键帧数目【小于10个】，则认为该地图中有效信息太少，直接重置，丢弃当前活跃地图
                            mpSystem->ResetActiveMap();
                            Verbose::PrintMess("Reseting current map...", Verbose::VERBOSITY_NORMAL);
                        }
                        else
                            // 情况2：当前活跃地图中的关键帧数目【超过10个】，则认为该地图有一定价值，存储起来作为非活跃地图，然后再创建一个新地图
                            CreateMapInAtlas();
...
#endif
            // 👇是从流程图的左下角的局部地图跟踪往后的过程了！
            // Step 7 在跟踪得到当前帧初始姿态后，现在对 local map 进行跟踪得到更多的匹配，并优化当前位姿
            ...
            if (mState == LOST)
            {
                // 情况1：如果地图中关键帧【小于10】，则重置当前地图，退出当前跟踪
                if (pCurrentMap->KeyFramesInMap() <= 10) // * 上一个版本这里是 5
                {
                    mpSystem->ResetActiveMap();
                    return;
                }
    ...
    bool Tracking::TrackReferenceKeyFrame()
    {
        ...
        // TODO check these condition
        // notice：这里不同于 ORB-SLAM2 的方式
        if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            return true;
        else
            return nmatchesMap >= 10; // 跟踪成功的数目超过【10】才认为跟踪成功，否则跟踪失败
    }
    ...
    bool Tracking::TrackWithMotionModel()
    {
        ...
        // Step 2：根据IMU或者恒速模型，得到当前帧的初始位姿。
        // notice：这里不同于 ORB-SLAM2 的方式
        if (mpAtlas->isImuInitialized() && (mCurrentFrame.mnId > mnLastRelocFrameId + mnFramesToResetIMU))
        {
            // Predict state with IMU if it is initialized and it doesnt need reset
            // IMU 完成初始化 并且 距离重定位挺久，那么不需要重置IMU，则用IMU来估计位姿，没有后面的这那那这的
            PredictStateIMU();
            return true;
        }
        else
    ...
    bool Tracking::TrackLocalMap()
    {
        ...
        else
        {
            ...
            else // 如果积累的IMU数据量比较多，考虑使用IMU数据优化
            {
                ...
                if (!mbMapUpdated) //  && (mnMatchesInliers>30))
                {
                    ...
                    // * 使用上一【普通帧】以及当前帧的视觉信息和IMU信息联合优化当前帧位姿、速度和IMU零偏
                    inliers = Optimizer::PoseInertialOptimizationLastFrame(&mCurrentFrame); // , !mpLastKeyFrame->GetMap()->GetIniertialBA1());
                }
                else
                {
                    Verbose::PrintMess("TLM: PoseInertialOptimizationLastKeyFrame ", Verbose::VERBOSITY_DEBUG);
                    // * 使用上一【关键帧】以及当前帧的视觉信息和IMU信息联合优化当前帧位姿、速度和IMU零偏
        ...
        // 如果最近刚刚发生了重定位，那么至少成功匹配 50 个点才认为是成功跟踪
            ...
            if (!vDepthIdx.empty())
            {
                ...
                for (size_t j = 0; j < vDepthIdx.size(); j++)
                {
                    ...
                    if (bCreateNew)
                    {
                        ...
                        // NOTE 这些添加属性的操作是每次创建MapPoint后都要做的！
    ...
    bool Tracking::Relocalization()
    {
        ...
        // Step 3：遍历所有的候选关键帧，通过BoW进行快速匹配，用匹配结果初始化 PnP Solver
        ...
        while (nCandidates > 0 && !bMatch)
        {
            ...
            for (int i = 0; i < nKFs; i++)
            {
                ...
                // Step 4.1：通过 MLPnP 算法估计姿态，迭代 5 次
                ...
                // bNoMore 为 true 表示已经超过了 RANSAC 最大迭代次数，就放弃当前关键帧
                if (bNoMore)
                {
                    vbDiscarded[i] = true;
                    nCandidates--;
                ...
                if (bTcw)
                {
                    // Step 4.2：如果 MLPnP 计算出了位姿，对内点进行BA优化
    ...
    void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame *pCurrentKeyFrame)
    {
        ...
        {
            // 当前帧需要预积分完毕，这段函数实在 localmapping 里调用的
            usleep(500);
        }

        // TODO 如果上一帧正好是上一帧的上一关键帧（mLastFrame.mpLastKeyFrame 与 mLastFrame 不可能是一个，可以验证一下）
